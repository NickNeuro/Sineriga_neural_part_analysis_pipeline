function GLM_INVESTMENT_II(subID, bids_subID, gen_path, mri_data_path, GLM_path)
...
extract_info_glm_investment_II(subID);

%% GLM preparation
names = {'avg_ret', 'vol', 'choice'};
onsets = cell(1,3);
durations = cell(1,3);
pmod_name = {'pmod_avg_ret', 'pmod_vol'};
pmod_value = cell(1,2);
...
for r = 1:runs 
    dataset = dir(fullfile(gen_path, '**', ['GLM_INVESTMENT_II_sub_' subID '_run_' num2str(r) '.mat'])); % information about the timing and values for the pmods
    ...
    onsets{1} = cat(1, onsets{1}, dataset.glm_data.avg_ret.onset + counter*param.n_scans*param.TR); % average return on investment of the volatile asset on trials when it's seen first
    onsets{2} = cat(1, onsets{2}, dataset.glm_data.vol.onset + counter*param.n_scans*param.TR); % volatility of the volatile asset on trials when it's seen second
    onsets{3} = cat(1, onsets{3}, dataset.glm_data.choice.onset + counter*param.n_scans*param.TR); % choice + safe asset screen for all trials
    counter = counter + 1;
    
    %% Durations
    durations{1} = cat(1, durations{1}, dataset.glm_data.avg_ret.duration);
    durations{2} = cat(1, durations{2}, dataset.glm_data.vol.duration);
    durations{3} = cat(1, durations{3}, dataset.glm_data.choice.duration); % reaction time

    %% Parametric modulators
    pmod_value{1} = cat(1, pmod_value{1}, dataset.glm_data.avg_ret.val'); % value of the average return on investment 
    pmod_value{2} = cat(1, pmod_value{2}, dataset.glm_data.vol.val'); % value of the average return on investment 
